<template>
  <div v-show="!showCount"
       class="dept-user-com-box dept-info">
    <div class="di-left">
      ...
      <div class="di-cell">
        <p>显示子级：</p>
        <el-switch v-model="isShowChildFlag"
                   inline-prompt
                   active-text="是"
                   inactive-text="否"
  ...
  <fs-crud ref="crudRef"
    ...
    <template #toolbar-left>
      <el-button :icon="!showCount ? 'Hide' : 'View'"
                 circle
                 @click="showCount = !showCount"></el-button>
    </template>
    <template #actionbar-right>
      <importExcel api="api/system/user/"
                   v-auth="'user:Import'">导入 </importExcel>
    </template>
  </fs-crud>

  <el-dialog v-model="resetPwdVisible"
             title="重设密码"
             width="400px"
             draggable
             :before-close="handleResetPwdClose">
    <div>
      <el-input v-model="resetPwdFormState.newPassword"
                type="password"
                placeholder="请输入密码"
                show-password
                style="margin-bottom: 20px" />
      <el-input v-model="resetPwdFormState.newPassword2"
                type="password"
                placeholder="请再次输入密码"
                show-password />
    </div>
    <template #footer>
      <span class="dialog-footer">
        ...
        <el-button type="primary"
                   @click="handleResetPwdSubmit"> 保存 </el-button>
      </span>
    </template>
  </el-dialog>
</template>

<template>
  <div class="home-container"
       v-if="reloadEchart">
    <el-row :gutter="15"
            class="home-card-one mb15">
      <el-col :xs="24"
              :sm="12"
              :md="12"
              :lg="6"
              :xl="6"
              v-for="(v, k) in homeOne"
              :key="k"
              :class="{ 'home-media home-media-lg': k > 1, 'home-media-sm': k === 1 }">
        <div class="home-card-item flex">
          <div class="flex-margin flex w100"
               :class="` home-one-animation${k}`">
            <div class="flex-auto">
              <span class="font30">{{ v.num1 }}</span>
              <div class="mt10">{{ v.num3 }}</div>
            </div>
            <div class="home-card-item-icon flex"
                 :style="{ background: `var(${v.color2})` }">
              <i class="flex-margin font32"
                 :class="v.num4"
                 :style="{ color: `var(${v.color3})` }"></i>
            </div>
          </div>
        </div>
      </el-col>
    </el-row>

    <el-row :gutter="15"
            class="home-card-two mb15">
      <el-col :xs="24"
              :sm="14"
              :md="14"
              :lg="16"
              :xl="16">
        <div class="home-card-item">
          <div style="padding-bottom:10px">
            <el-radio-group v-model="weekLineRadio"
                            @change="changeWeekData">
              <el-radio-button label="新增"
                               value="1" />
              <el-radio-button label="下线"
                               value="2" />
            </el-radio-group>
          </div>
          <div style="height: 90%"
               ref="weekLineRef"></div>
        </div>
      </el-col>
      <el-col :xs="24"
              :sm="10"
              :md="10"
              :lg="8"
              :xl="8"
              class="home-media">
        <div class="home-card-item">
          <div style="height: 100%"
               ref="centerIpPieRef"></div>
        </div>
      </el-col>
    </el-row>
    <el-row :gutter="15"
            class="home-card-two mb15">
      <el-col :xs="24"
              :sm="14"
              :md="14"
              :lg="16"
              :xl="16">
        <div class="home-card-item">
          <div style="padding-bottom:10px">
            <el-radio-group v-model="monthLineRadio"
                            @change="changeMonthData">
              <el-radio-button label="新增"
                               value="1" />
              <el-radio-button label="下线"
                               value="2" />
            </el-radio-group>
          </div>
          <div style="height: 90%"
               ref="monthLineRef"></div>
        </div>
      </el-col>
      <el-col :xs="24"
              :sm="10"
              :md="10"
              :lg="8"
              :xl="8"
              class="home-media">
        <div class="home-card-item">
          <div style="height: 100%"
               ref="centerBizPieRef"></div>
        </div>
      </el-col>
    </el-row>

    <el-row :gutter="15"
            class="home-card-three">
      <el-col :xs="24"
              :sm="24"
              :md="24"
              :lg="24"
              :xl="24"
              class="home-media">
        <div class="home-card-item"
             style="height:96.5vh">
          <div style="padding-bottom:10px">
            <el-radio-group v-model="assetRadio"
                            @change="changeAssetRadio">
              <el-radio-button label="各个公司"
                               value="1" />
              <el-radio-button label="现存未知"
                               value="2" />
            </el-radio-group>
          </div>
          <div style="height: 100%"
               ref="homeBarRef"></div>
        </div>
      </el-col>
    </el-row>
  </div>
</template>

<script lang="ts">
import { toRefs, reactive, defineComponent, onMounted, ref, watch, nextTick, onActivated } from 'vue';
import * as echarts from 'echarts';
import { storeToRefs } from 'pinia';
import { useThemeConfig } from '@/stores/themeConfig';
import { useTagsViewRoutes } from '@/stores/tagsViewRoutes';
import apis from './apis';
import { forEach } from 'lodash';

let global: any = {
	monthLineRef: null,
	weekLineRef: null,
	centerBizRef: null,
	centerIpRef: null,
	assetBarChart: null,
	dispose: [null, '', undefined],
};

export default defineComponent({
	name: 'home',
	setup() {
		const monthLineRef = ref();
		const weekLineRef = ref();
		const centerIpPieRef = ref();
		const centerBizPieRef = ref();
		const homeBarRef = ref();
		const storesTagsViewRoutes = useTagsViewRoutes();
		const storesThemeConfig = useThemeConfig();
		const { themeConfig } = storeToRefs(storesThemeConfig);
		const { isTagsViewCurrenFull } = storeToRefs(storesTagsViewRoutes);
		const state = reactive({
			homeOne: [
				{
					num1: '0',
					num3: 'XXXX',
					num4: 'iconfont icon-diannao101',
					color1: '#6690F9',
					color2: '--next-color-primary-lighter',
					color3: '--el-color-primary',
				},
				{
					num1: '0',
					num3: 'XXXX',
					num4: 'iconfont icon-LoggedinPC',
					color1: '#6690F9',
					color2: '--next-color-success-lighter',
					color3: '--el-color-success',
				},
				{
					num1: '0',
					num3: 'XXXX',
					num4: 'iconfont icon-putong',
					color1: '#6690F9',
					color2: '--next-color-warning-lighter',
					color3: '--el-color-warning',
				},
				{
					num1: '0',
					num3: 'XXXX',
					num4: 'iconfont icon-zujian',
					color1: '#FF6462',
					color2: '--next-color-danger-lighter',
					color3: '--el-color-danger',
				},
			],
			myCharts: [],
			charts: {
				theme: themeConfig.value.isIsDark ? 'dark' : '',
				bgColor: themeConfig.value.isIsDark ? 'transparent' : '',
				color: themeConfig.value.isIsDark ? '#dadada' : '#303133',
			},
		});
		// 折线图
		const initLineChart = (title: string, containerRef: any, xData: any, yData: any, type: string) => {
			if (global.weekLineRef && type === 'week') {
				global.weekLineRef.dispose();
			}
			if (global.monthLineRef && type === 'month') {
				global.monthLineRef.dispose();
			}
			if (type === 'week') {
				global.weekLineRef = <any>echarts.init(containerRef.value, state.charts.theme);
				const option = {
					backgroundColor: state.charts.bgColor,
					title: {
						text: title,
						x: 'left',
						textStyle: { fontSize: '15', color: state.charts.color },
					},
					grid: { top: 70, right: 20, bottom: 30, left: 30 },
					tooltip: { trigger: 'axis' },
					legend: { data: yData.legend, left: '200', right: '10', type: 'scroll' },
					xAxis: {
						data: xData,
					},
					yAxis: [
						{
							type: 'value',
							name: '数量',
							splitLine: { show: true, lineStyle: { type: 'dashed', color: '#f5f5f5' } },
						},
					],
					series: yData.series,
				};
				(<any>global.weekLineRef).setOption(option);
				state.myCharts.push(global.weekLineRef);
			} else {
				global.monthLineRef = <any>echarts.init(containerRef.value, state.charts.theme);
				const option = {
					backgroundColor: state.charts.bgColor,
					title: {
						text: title,
						x: 'left',
						textStyle: { fontSize: '15', color: state.charts.color },
					},
					grid: { top: 70, right: 20, bottom: 30, left: 30 },
					tooltip: { trigger: 'axis' },
					legend: { data: yData.legend, left: '200', right: '10', type: 'scroll' },
					xAxis: {
						data: xData,
					},
					yAxis: [
						{
							type: 'value',
							name: '数量',
							splitLine: { show: true, lineStyle: { type: 'dashed', color: '#f5f5f5' } },
						},
					],
					series: yData.series,
				};
				(<any>global.monthLineRef).setOption(option);
				state.myCharts.push(global.monthLineRef);
			}
		};
		// 饼图
		const initPieChart = (title, chartContainer: any, getname: any, data: any, type: string) => {
			if (type === 'biz' && global.centerBizRef) {
				global.centerBizRef.dispose();
			}
			if (type === 'host' && global.centerIpRef) {
				global.centerIpRef.dispose();
			}
			if (type === 'biz') {
				global.centerBizRef = <any>echarts.init(chartContainer, state.charts.theme);
				const option = {
					backgroundColor: state.charts.bgColor,
					title: {
						text: title,
						subtext: '数据中心',
						x: 'left',
						textStyle: { fontSize: '15', color: state.charts.color },
					},
					tooltip: { trigger: 'item', formatter: '{b} <br/> {c}' },
					graphic: {
						elements: [
							{
								type: 'image',
								z: -1,
								style: {
									image: themeConfig.value.isIsDark
										? ''
										: 'data:image/png;base64,',
									width: 230,
									height: 230,
								},
								left: '16.5%',
								top: 'center',
							},
						],
					},
					legend: {
						type: 'scroll',
						orient: 'vertical',
						left: '65%',
						top: 'center',
						itemWidth: 11,
						itemHeight: 11,
						data: getname,
						formatter: function (name) {
							return echarts.format.truncateText(name, 100, '14px Microsoft Yahei', '…');
						},
					},
					series: [
						{
							type: 'pie',
							radius: ['82', themeConfig.value.isIsDark ? '50' : '102'],
							center: ['32%', '50%'],
							label: { show: false },
							labelLine: { show: false },
							data: data,
						},
					],
				};
				(<any>global.centerBizRef).setOption(option);
				state.myCharts.push(global.centerBizRef);
			} else {
				global.centerIpRef = <any>echarts.init(chartContainer, state.charts.theme);
				const option = {
					backgroundColor: state.charts.bgColor,
					title: {
						text: title,
						subtext: '数据中心',
						x: 'left',
						textStyle: { fontSize: '15', color: state.charts.color },
					},
					tooltip: { trigger: 'item', formatter: '{b} <br/> {c}' },
					graphic: {
						elements: [
							{
								type: 'image',
								z: -1,
								style: {
									image: themeConfig.value.isIsDark
										? ''
										: 'data:image/png;base64,',
									width: 230,
									height: 230,
								},
								left: '16.5%',
								top: 'center',
							},
						],
					},
					legend: {
						type: 'scroll',
						orient: 'vertical',
						left: '65%',
						top: 'center',
						itemWidth: 11,
						itemHeight: 11,
						data: getname,
						formatter: function (name) {
							return echarts.format.truncateText(name, 100, '14px Microsoft Yahei', '…');
						},
					},
					series: [
						{
							type: 'pie',
							radius: ['82', themeConfig.value.isIsDark ? '50' : '102'],
							center: ['32%', '50%'],
							label: { show: false },
							labelLine: { show: false },
							data: data,
						},
					],
				};
				(<any>global.centerIpRef).setOption(option);
				state.myCharts.push(global.centerIpRef);
			}
		};
		// 柱状图
		const labelOption = {
			show: true,
			position: 'insideBottom',
			distance: 5,
			align: 'left',
			verticalAlign: 'middle',
			rotate: 90,
			formatter: '{name|{a}}',
			fontSize: 11,
			rich: {
				name: {},
			},
		};
		const initBarChart = (data: any) => {
			if (global.assetBarChart) {
				global.assetBarChart.dispose();
			}
			global.assetBarChart = <any>echarts.init(homeBarRef.value, state.charts.theme);

			const option = {
				backgroundColor: state.charts.bgColor,
				title: {
					text: '资产统计',
					x: 'left',
					textStyle: { fontSize: '15', color: state.charts.color },
				},
				tooltip: {
					trigger: 'axis',
					axisPointer: {
						type: 'shadow',
					},
				},
				legend: {
					data: data.legend,
					left: '200',
					right: '10',
					type: 'scroll',
				},
				toolbox: {
					show: true,
					orient: 'vertical',
					left: 'right',
					top: 'center',
					feature: {
						magicType: { show: true, type: ['line', 'bar'] },
						saveAsImage: { show: true },
					},
				},
				xAxis: [
					{
						type: 'category',
						axisTick: { show: false },
						data: data.xAxis,
					},
				],
				yAxis: [
					{
						type: 'value',
					},
				],
				series: data.series,
			};
			(<any>global.assetBarChart).setOption(option);
			(<any>state.myCharts).push(global.assetBarChart);
		};
		let newMonthChartData = {};
		let offlineMonthChartData = {};
		let newWeekChartData = {};
		let offlineWeekChartData = {};
		let dataCenterData = {};
		let companyData = {};
		interface NameValue {
			name: string;
			value: number;
		}
		let centerBizName = [];
		let centerBizData = [];
		let centerIpName = [];
		let centerIpData = [];

		const weekLineRadio = ref('1');
		const monthLineRadio = ref('1');
		const assetRadio = ref('1');
		const changeWeekData = (val: string) => {
			if (val === '1') {
				initLineChart(newWeekChartData.title, newWeekChartData.weekLineRef, newWeekChartData.xData, newWeekChartData.data, 'week');
			} else {
				initLineChart(offlineWeekChartData.title, offlineWeekChartData.weekLineRef, offlineWeekChartData.xData, offlineWeekChartData.data, 'week');
			}
		};
		const changeMonthData = (val: string) => {
			if (val === '1') {
				initLineChart(newMonthChartData.title, newMonthChartData.monthLineRef, newMonthChartData.xData, newMonthChartData.data, 'month');
			} else {
				initLineChart(
					offlineMonthChartData.title,
					offlineMonthChartData.monthLineRef,
					offlineMonthChartData.xData,
					offlineMonthChartData.data,
					'month'
				);
			}
		};
		const changeAssetRadio = (val: string) => {
			if (val === '1') {
				initBarChart(companyData);
			} else {
				initBarChart(dataCenterData);
			}
		};
		let reloadEchart = ref(true);
		watch(
			() => themeConfig.value.isCollapse,
			(isCollapse) => {
				reloadEchart.value = false;
				nextTick(() => {
					reloadEchart.value = true;
					let timer = setTimeout(() => {
						initPieChart('主机数量统计', centerIpPieRef.value, centerBizName, centerBizData, 'host');
						initPieChart('业务系统统计', centerBizPieRef.value, centerIpName, centerIpData, 'biz');
						changeWeekData(weekLineRadio.value);
						changeMonthData(monthLineRadio.value);
						changeAssetRadio(assetRadio.value);
						clearTimeout(timer);
					}, 500);
				});
			}
		);
		watch(
			() => themeConfig.value.isIsDark,
			(isIsDark) => {
				state.charts.theme = isIsDark ? 'dark' : '';
				state.charts.bgColor = isIsDark ? 'transparent' : '';
				state.charts.color = isIsDark ? '#dadada' : '#303133';
				initPieChart('主机数量统计', centerIpPieRef.value, centerBizName, centerBizData, 'host');
				initPieChart('业务系统统计', centerBizPieRef.value, centerIpName, centerIpData, 'biz');
				changeWeekData(weekLineRadio.value);
				changeMonthData(monthLineRadio.value);
				changeAssetRadio(assetRadio.value);
			}
		);
		const initCharts = () => {
			apis.getHostFoldline().then((res: any) => {
				const monthDataNew = res.data?.month_data?.new_system;
				const monthDataOffline = res.data?.month_data?.offline_system;
				const weekDataNew = res.data?.week_data?.new_system;
				const weekDataOffline = res.data?.week_data?.offline_system;
				if (monthDataNew && monthDataOffline) {
					let seriesNew = [];
					let legendNew = Object.keys(monthDataNew);
					let xDataNew = [];
					legendNew.forEach((k: string) => {
						let obj = {
							name: '',
							type: 'line',
							smooth: true,
							data: [],
						};
						monthDataNew[k].forEach((v: any) => {
							if (!xDataNew.includes(v.name)) {
								xDataNew.push(v.name);
							}
							obj.name = k;
							obj.data.push(v.value);
						});
						seriesNew.push(obj);
					});
					newMonthChartData = {
						title: '业务系统数量统计(月)',
						monthLineRef,
						xData: xDataNew,
						data: {
							legend: legendNew,
							series: seriesNew,
						},
					};
					initLineChart(newMonthChartData.title, newMonthChartData.monthLineRef, newMonthChartData.xData, newMonthChartData.data, 'month');
					let seriesOffine = [];
					let legendOffine = Object.keys(monthDataOffline);
					let xDataOffine = [];
					legendOffine.forEach((k: string) => {
						let obj = {
							name: '',
							type: 'line',
							smooth: true,
							data: [],
						};
						monthDataOffline[k].forEach((v: any) => {
							if (!xDataOffine.includes(v.name)) {
								xDataOffine.push(v.name);
							}
							obj.name = k;
							obj.data.push(v.value);
						});
						seriesOffine.push(obj);
					});
					offlineMonthChartData = {
						title: '业务系统数量统计(月)',
						monthLineRef,
						monthLine: global.monthLineRef,
						xData: xDataOffine,
						data: {
							legend: legendOffine,
							series: seriesOffine,
						},
					};
				}
				if (weekDataNew && weekDataOffline) {
					let seriesNew = [];
					let legendNew = Object.keys(weekDataNew);
					let xDataNew = [];
					legendNew.forEach((k: string) => {
						let obj = {
							name: '',
							type: 'line',
							smooth: true,
							data: [],
						};
						weekDataNew[k].forEach((v: any) => {
							if (!xDataNew.includes(v.name)) {
								xDataNew.push(v.name);
							}
							obj.name = k;
							obj.data.push(v.value);
						});
						seriesNew.push(obj);
					});
					newWeekChartData = {
						title: '业务系统数量统计(周)',
						weekLineRef,
						xData: xDataNew,
						data: {
							legend: legendNew,
							series: seriesNew,
						},
					};
					initLineChart(newWeekChartData.title, newWeekChartData.weekLineRef, newWeekChartData.xData, newWeekChartData.data, 'week');
					let seriesOffine = [];
					let legendOffine = Object.keys(weekDataOffline);
					let xDataOffine = [];
					legendOffine.forEach((k: string) => {
						let obj = {
							name: '',
							type: 'line',
							smooth: true,
							data: [],
						};
						weekDataOffline[k].forEach((v: any) => {
							if (!xDataOffine.includes(v.name)) {
								xDataOffine.push(v.name);
							}
							obj.name = k;
							obj.data.push(v.value);
						});
						seriesOffine.push(obj);
					});
					offlineWeekChartData = {
						title: '业务系统数量统计(周)',
						weekLineRef,
						xData: xDataOffine,
						data: {
							legend: legendOffine,
							series: seriesOffine,
						},
					};
				}
			});
			apis.getHostCount().then((res: any) => {
				centerBizName = Object.keys(res.data.center_biz);
				centerBizData = Object.values(res.data.center_biz).map((v, i) => {
					return { name: centerBizName[i], value: v };
				});
				centerIpName = Object.keys(res.data.center_ip);
				centerIpData = Object.values(res.data.center_ip).map((v, i) => {
					return { name: centerIpName[i], value: v };
				});
				initPieChart('主机数量统计', centerIpPieRef.value, centerBizName, centerBizData, 'host');
				initPieChart('业务系统统计', centerBizPieRef.value, centerIpName, centerIpData, 'biz');
				if (Object.values(res.data.assets_count).length !== 4) return;
				let index = 0;
				for (const name in res.data.assets_count) {
					const value = res.data.assets_count[name];
					state.homeOne[index].num1 = value;
					state.homeOne[index].num3 = name;
					index++;
				}
			});
			apis.getColumnar().then((res: any) => {
				const company = res.data?.company;
				const dataCenter = res.data?.data_center;
				let data = {
					legend: [],
					xAxis: [],
					series: [],
				};

				if (company) {
					data.legend = Object.keys(company);
					data.legend.forEach((k: string) => {
						let obj = {
							name: '',
							type: 'bar',
							barGap: 0,
							label: labelOption,
							emphasis: {
								focus: 'series',
							},
							data: [],
						};
						company[k].forEach((v: NameValue) => {
							if (!data.xAxis.includes(v.name)) {
								data.xAxis.push(v.name);
							}
							obj.data.push(v.value);
						});
						obj.name = k;
						data.series.push(obj);
					});
					companyData = data;
					initBarChart(companyData);
				}
				if (dataCenter) {
					data = {
						legend: [],
						xAxis: [],
						series: [],
					};
					data.legend = Object.keys(dataCenter);
					data.legend.forEach((k: string) => {
						let obj = {
							name: '',
							type: 'bar',
							barGap: 0,
							label: labelOption,
							emphasis: {
								focus: 'series',
							},
							data: [],
						};
						dataCenter[k].forEach((v: NameValue) => {
							if (!data.xAxis.includes(v.name)) {
								data.xAxis.push(v.name);
							}
							obj.data.push(v.value);
						});
						obj.name = k;
						data.series.push(obj);
					});
					dataCenterData = data;
				}
			});
		};
		initCharts();

		return {
			monthLineRef,
			weekLineRef,
			weekLineRadio,
			monthLineRadio,
			centerIpPieRef,
			centerBizPieRef,
			homeBarRef,
			...toRefs(state),
			changeWeekData,
			changeMonthData,
			changeAssetRadio,
			assetRadio,
			reloadEchart,
		};
	},
});
</script>

<style scoped lang="scss">
$homeNavLengh: 8;
.home-container {
	overflow: hidden;
	padding: 15px;
	.home-card-one,
	.home-card-two,
	.home-card-three {
		.home-card-item {
			width: 100%;
			height: 130px;
			border-radius: 4px;
			transition: all ease 0.3s;
			padding: 20px;
			overflow: hidden;
			background: var(--el-color-white);
			color: var(--el-text-color-primary);
			border: 1px solid var(--next-border-color-light);
			&:hover {
				box-shadow: 0 2px 12px var(--next-color-dark-hover);
				transition: all ease 0.3s;
			}
			&-icon {
				width: 70px;
				height: 70px;
				border-radius: 100%;
				flex-shrink: 1;
				i {
					color: var(--el-text-color-placeholder);
				}
			}
			&-title {
				font-size: 15px;
				font-weight: bold;
				height: 30px;
			}
		}
	}
	.home-card-one {
		@for $i from 0 through 3 {
			.home-one-animation#{$i} {
				opacity: 0;
				animation-name: error-num;
				animation-duration: 0.5s;
				animation-fill-mode: forwards;
				animation-delay: calc($i/10) + s;
			}
		}
	}
	.home-card-two,
	.home-card-three {
		.home-card-item {
			height: 500px;
			width: 100%;
			overflow: hidden;
			.home-monitor {
				height: 100%;
				.flex-warp-item {
					width: 25%;
					height: 111px;
					display: flex;
					.flex-warp-item-box {
						margin: auto;
						text-align: center;
						color: var(--el-text-color-primary);
						display: flex;
						border-radius: 5px;
						background: var(--next-bg-color);
						cursor: pointer;
						transition: all 0.3s ease;
						&:hover {
							background: var(--el-color-primary-light-9);
							transition: all 0.3s ease;
						}
					}
					@for $i from 0 through $homeNavLengh {
						.home-animation#{$i} {
							opacity: 0;
							animation-name: error-num;
							animation-duration: 0.5s;
							animation-fill-mode: forwards;
							animation-delay: calc($i/10) + s;
						}
					}
				}
			}
		}
	}
}
</style>

<template>
  <div>
    ...
    <el-form ref="formRef"
      ...
      <el-form-item :prop="['array'].indexOf(item.form_item_type_label) > -1 ? '' : item.key"
                    :key="index"
                    :rules="item.rule || []"
                    v-for="(item, index) in formList">
        <el-col :span="4">
          <el-input v-if="item.edit"
                    v-model="item.title"
        ...
        <el-col :span="10">
          ...
          <el-input-number :key="index + 1"
                           v-else-if="item.form_item_type_label === 'number'"
                           v-model="formData[item.key]"
          ...
          <!--    select      -->
          <el-select :key="index + 3"
                     v-else-if="item.form_item_type_label === 'select'"
                     v-model="formData[item.key]"
                     :placeholder="item.placeholder"
                     clearable>
            <el-option v-for="item in dictionary(item.setting) || []"
                       :key="item.value"
                       :label="item.label"
                       :value="item.value"> </el-option>
          </el-select>
          <!--    checkbox      -->
          <el-checkbox-group :key="index + 4"
                             v-else-if="item.form_item_type_label === 'checkbox'"
                             v-model="formData[item.key]"
                             :placeholder="item.placeholder">
            <el-checkbox v-for="item in dictionary(item.setting) || []"
                         :key="item.value"
                         :label="item.value"
                         :value="item.value">
              {{ item.label }}
          ...
          <!--    radio      -->
          <el-radio-group :key="index + 5"
                          v-else-if="item.form_item_type_label === 'radio'"
                          v-model="formData[item.key]"
                          :placeholder="item.placeholder"
                          clearable>
            <el-radio v-for="item in dictionary(item.setting) || []"
                      :key="item.value"
                      :label="item.value"
                      :value="item.value">
              {{ item.label }}
          ...
          <!--     图片     -->
          <div v-else-if="['img', 'imgs'].indexOf(item.form_item_type_label) > -1"
               :key="index + 7">
            <el-upload :action="uploadUrl"
                ...
                "
                       ...
                       :before-remove="(file: any, fileList: any) => {
    ...
    "
              ...
              <div slot="tip"
                   class="el-upload__tip">请选取图片,并且只能上传jpg/png文件</div>
            </el-upload>
          ...
          <!--     文件     -->
          <div v-else-if="['file'].indexOf(item.form_item_type_label) > -1"
               :key="index + 8">
            <el-upload :action="uploadUrl"
                       :headers="uploadHeaders"
                       name="file"
    ...
    "
          ...
          <!--    关联表      -->
          <div v-else-if="['foreignkey', 'manytomany'].indexOf(item.form_item_type_label) > -1"
               :key="index + 9">
            <table-selector v-model="formData[item.key]"
                            :el-props="{
								pagination: true,
                            ...
                            :pagination="true"
                            :multiple="item.form_item_type_label === 'manytomany'"></table-selector>
          </div>
          <!--   数组       -->
          <div v-else-if="item.form_item_type_label === 'array'"
               :key="index + 10">
            <vxe-table border
                       resizable
                       auto-resize
                  ...
                  <el-popover placement="top"
                    ...
                    <div style="text-align: right; margin: 0">
                      <el-button size="small"
                                 link
                                 @click="childRemoveVisible = false">取消</el-button>
                      <el-button type="primary"
                                 size="small"
                                 @click="onRemoveChild(row, index, item.key)">确定</el-button>
                    </div>
                    <el-button link
            ...
            <div>
              <el-button size="small"
                         @click="onAppend('xTable_' + item.key)">追加</el-button>
            </div>
          </div>
        </el-col>
        <el-col :span="2"
        ...
        <el-col :span="3">
          <el-button v-if="item.edit"
                     size="small"
                     type="primary"
                     :icon="Finished"
                     @click="onEditSave(item)">保存</el-button>
          <el-button v-else
                     size="small"
                     type="primary"
                     :icon="Edit"
                     @click="onEdit(index)"></el-button>
      ...
      <el-form-item>
        <el-button type="primary"
                   @click="onSubmit(formRef)">确定</el-button>
      </el-form-item>
    </el-form>
  </div>
</template>

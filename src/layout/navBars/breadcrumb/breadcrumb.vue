  ...
  <div v-if="isShowBreadcrumb"
    ...
    <el-breadcrumb class="layout-navbars-breadcrumb-hide">
      <transition-group name="breadcrumb">
        <el-breadcrumb-item key="ljfaljda*2">
...
const getBreadcrumbList = (arr: RouteItems) => {
	arr.forEach((item: RouteItem) => {
		state.routeSplit.forEach((v: string) => {
			// console.log(item);
...
let breadcrumbList = ref<RouteItems>([]);
// 页面加载时
onMounted(() => {
	if (route.path == '/home') {
	...
	breadcrumbList.value = [route];
	// getBreadcrumbList(route.matched);
});
// 路由更新时

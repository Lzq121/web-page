  ...
  <div class="layout-navbars-tagsview"
    ...
    <el-scrollbar ref="scrollbarRef"
      ...
      <ul class="layout-navbars-tagsview-ul"
          :class="setTagsStyle"
          ref="tagsUlRef">
        <li v-for="(v, k) in state.tagsViewList"
            :key="k"
            ...
            :ref="
					...
					">
          <i class="iconfont icon-webicon318 layout-navbars-tagsview-ul-li-iconfont"
             v-if="isActive(v)"></i>
...
const addTagsView = (path: string, to?: RouteToFrom) => {
	...
	nextTick(async () => {
		...
		let item: RouteItem;
		// if (Array.isArray(state.tagsViewList) && state.tagsViewList.length > 10) state.tagsViewList.shift();
